...
if [[ -f "${ENV_SH}" && -f "${UTIL_SH}" ]]; then
  ...
  echo "ERROR: Helper libraries not found in examples/harden/lib/." >&2
  exit 1
fi
...
if [[ "${VM_WAS_CREATED}" == "true" ]]; then
  ...
  IAP_READY=1
  for i in {1..36}; do
    if bash "${BIN_DIR}/ssh-debug-vm.sh" "uptime" &>/dev/null; then
      IAP_READY=0
      break
    fi
...
if [[ "${CLEAN_BUILD}" -eq 1 ]]; then
  ...
  gcloud compute ssh "${INSTANCE_NAME}" --zone "${ZONE}" --project "${PROJECT_ID}" --tunnel-through-iap --ssh-flag="-oControlMaster=no" --ssh-flag="-oBatchMode=yes" --ssh-flag="-oConnectTimeout=5" --command "
...
set +e
gcloud compute ssh "${INSTANCE_NAME}" --zone "${ZONE}" --project "${PROJECT_ID}" --tunnel-through-iap --ssh-flag="-oControlMaster=no" --ssh-flag="-oBatchMode=yes" --ssh-flag="-oConnectTimeout=5" --command "screen -ls | grep -q customization" &>/dev/null
...
EOF
  ...
  # Connect and attach
  gcloud compute ssh "${INSTANCE_NAME}" --zone "${ZONE}" --project "${PROJECT_ID}" --tunnel-through-iap -- -t screen -rxU customization
  ...
  cp "${DATAPROC_EVOLUTION_DIR}/custom-images/examples/harden/lib/env.sh" "${LOCAL_STAGING}/env.sh"
  cp "${DATAPROC_EVOLUTION_DIR}/custom-images/examples/harden/lib/util.sh" "${LOCAL_STAGING}/util.sh"
  cp "${DATAPROC_EVOLUTION_DIR}/custom-images/examples/harden/install-in-screen.sh" "${LOCAL_STAGING}/install-in-screen.sh"
  ...
  gcloud compute ssh "${INSTANCE_NAME}" --zone "${ZONE}" --project "${PROJECT_ID}" --tunnel-through-iap --ssh-flag="-oControlMaster=no" --ssh-flag="-oBatchMode=yes" --ssh-flag="-oConnectTimeout=10" --command "
  ...
  set +e
  gcloud compute ssh "${INSTANCE_NAME}" --zone "${ZONE}" --project "${PROJECT_ID}" --tunnel-through-iap --ssh-flag="-oControlMaster=no" --ssh-flag="-oBatchMode=yes" --ssh-flag="-oConnectTimeout=3" --command "[[ -f /tmp/sources/customization.exit ]]" &>/dev/null
  ...
  if [[ ${attach_success} -ne 0 ]]; then
    ...
    set +e
    # Fetch exit code
    exit_code=$(gcloud compute ssh "${INSTANCE_NAME}" --zone "${ZONE}" --project "${PROJECT_ID}" --tunnel-through-iap --ssh-flag="-oBatchMode=yes" --command "cat /tmp/sources/customization.exit" 2>/dev/null)
    ...
    if [[ ${has_exit} -eq 0 && -n "${exit_code}" ]]; then
      ...
      echo "========================================================================" >&2
      gcloud compute ssh "${INSTANCE_NAME}" --zone "${ZONE}" --project "${PROJECT_ID}" --tunnel-through-iap --ssh-flag="-oBatchMode=yes" --command "cat /tmp/sources/customization-output.log" || true
...
set +e
EXIT_CODE=$(gcloud compute ssh "${INSTANCE_NAME}" --zone "${ZONE}" --project "${PROJECT_ID}" --tunnel-through-iap --ssh-flag="-oControlMaster=no" --ssh-flag="-oConnectTimeout=5" --command "cat /tmp/sources/customization.exit" 2>/dev/null)
...
if [[ ${SSH_STATUS} -ne 0 || -z "${EXIT_CODE}" ]]; then
  ...
  set +e
  screen_still_active=$(gcloud compute ssh "${INSTANCE_NAME}" --zone "${ZONE}" --project "${PROJECT_ID}" --tunnel-through-iap --ssh-flag="-oControlMaster=no" --ssh-flag="-oConnectTimeout=5" --command "screen -ls | grep -q customization" 2>/dev/null; echo $?)
...
To re-attach and monitor the live run later, simply run this script again:
  ...
  bash examples/harden/bin/customize-in-screen.sh
========================================================================
EOF
    exit 0
  else
...
1. View the full build log:
   bash examples/harden/bin/ssh-debug-vm.sh "cat /tmp/sources/customization-output.log"

2. Hot-fix the script directly on the VM for instant testing:
   bash examples/harden/bin/ssh-debug-vm.sh
