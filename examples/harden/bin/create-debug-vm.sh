...
# Do not override SA_NAME, use what env.sh provides
# In this context, env.sh is already sourced and provides GSA.
# Let's ensure we use the GSA from env.sh directly.
...
run_gsutil cp_env gsutil cp "${REPO_ROOT}/examples/harden/lib/env.sh" "${GCS_SOURCES_PATH}/env.sh"
run_gsutil cp_util gsutil cp "${REPO_ROOT}/examples/harden/lib/util.sh" "${GCS_SOURCES_PATH}/util.sh"
