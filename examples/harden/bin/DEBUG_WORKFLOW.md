...
Located in `examples/harden/bin/`:

*   **[`customize-in-screen.sh`](file:///usr/local/google/home/cjac/src/github/c9h/dataproc-evolution/custom-images/examples/harden/bin/customize-in-screen.sh):** The main workstation-side orchestrator.
*   **[`create-debug-vm.sh`](file:///usr/local/google/home/cjac/src/github/c9h/dataproc-evolution/custom-images/examples/harden/bin/create-debug-vm.sh):** Helper to provision the high-spec (`n1-standard-32`) builder VM.
*   **[`install-in-screen.sh`](file:///usr/local/google/home/cjac/src/github/c9h/dataproc-evolution/custom-images/examples/harden/install-in-screen.sh):** The guest-side wrapper that spawns the script in `screen`.
...
### 1. Configure Target
Ensure your `env.json` points to the customization script you want to test (e.g., `examples/harden/harden-kernel-and-os.sh`).
...
3.  **Clean Builds:** Pass `-c` to clear sentinels and artifcats before running:
    ```bash
    bash examples/harden/bin/customize-in-screen.sh -c
    ```
