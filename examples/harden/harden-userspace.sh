...
function run_lynis_scan() {
    print_status "Running Lynis compliance scan..."
    # Ensure Lynis is installed
    if ! command -v lynis >/dev/null 2>&1; then
        apt-get update && apt-get install -y lynis || print_warning "Failed to install Lynis"
    fi
    if command -v lynis >/dev/null 2>&1; then
        lynis audit system --quick || print_warning "Lynis scan encountered warnings/errors (see /var/log/lynis-report.dat)"
        print_success "Lynis compliance scan completed."
    else
        print_warning "Skipping Lynis scan, executable not found."
    fi
}

function run_cve_detector() {
    print_status "Running CVE detection pass (Trivy & debsecan)..."
    
    # 1. Debsecan (Debian native)
    if ! command -v debsecan >/dev/null 2>&1; then
        apt-get install -y debsecan || print_warning "Failed to install debsecan"
    fi
    if command -v debsecan >/dev/null 2>&1; then
        print_status "Executing debsecan..."
        debsecan --suite=bookworm --format=summary || print_warning "debsecan execution failed"
    fi

    # 2. Trivy (Industry standard fs scanner)
    if ! command -v trivy >/dev/null 2>&1; then
        print_status "Installing Trivy..."
        curl -sfL https://raw.githubusercontent.com/aquasecurity/trivy/main/contrib/install.sh | sh -s -- -b /usr/local/bin || print_warning "Failed to install Trivy"
    fi
    if command -v trivy >/dev/null 2>&1; then
        print_status "Executing trivy fs scan..."
        trivy fs / --severity HIGH,CRITICAL --no-progress --timeout 15m || print_warning "Trivy scan execution failed or timed out"
    fi
    
    print_success "CVE detection pass completed."
}
