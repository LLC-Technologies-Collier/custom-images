...
IMAGE_PROJECT="$(jq -r .IMAGE_PROJECT              "${ENV_JSON_PATH}")"
if [[ "${IMAGE_PROJECT}" == "null" ]]; then
  IMAGE_PROJECT="cloud-dataproc"
fi
MACHINE_TYPE="$(jq -r .MACHINE_TYPE                "${ENV_JSON_PATH}")"
if [[ "${MACHINE_TYPE}" == "null" ]]; then
...
export PROJECT_ID PURPOSE BUCKET TEMP_BUCKET ZONE SUBNET HIVE_NAME HIVEDB_PW_URI SECRET_NAME KMS_KEY_URI PRINCIPAL_USER DOMAIN IMAGE_VERSION CUSTOMIZATION_SCRIPT IMAGE_PROJECT MACHINE_TYPE
